  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden bg-muted/40">
        ...
        {off > 0 && (
          <span className="absolute left-2 top-2 rounded-full bg-[#962E3C] px-2 py-0.5 text-[10px] font-bold text-white">
            {off}% OFF
          </span>
        )}
      </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-lg bg-background pb-10">
        ...
        <div className="flex items-center justify-center border-b border-border py-3">
          <img src={LOGO} alt="Beauty Barn" className="h-7 w-auto object-contain" />
        </div>

        {/* Creator profile */}
        <div className="px-4 pt-5">
          <div className="flex items-start gap-4">
            <div className="h-[76px] w-[76px] shrink-0 overflow-hidden rounded-full border-2 border-[#962E3C] p-0.5">
              {creator.avatar_url ? (
                <img src={creator.avatar_url} alt={creator.name} className="h-full w-full rounded-full object-cover" />
              ) : (
                <div className="flex h-full w-full items-center justify-center rounded-full bg-[#962E3C] text-xl font-bold text-white">
                  {initials}
                </div>
              )}
            </div>
              ...
              {categories.map((c) => (
                <button
                  ...
                  className={cn(
                    ...
                    category === c
                      ? "border-[#962E3C] bg-[#962E3C] font-medium text-white"
        ...
        <div className="px-4 pt-8 text-center">
          <p className="text-xs text-muted-foreground">
            ...
            <span className="font-semibold text-[#962E3C]">BeautyBarn</span>
          </p>
        </div>
      </div>
    </div>
