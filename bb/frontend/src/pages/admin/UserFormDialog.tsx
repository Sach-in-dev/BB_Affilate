...
export function UserFormDialog({
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      setRoleName(first);
      setSelectedPerms(roles[0]?.permissions ?? []);
    }
  }, [open, editing, roles]);

  const handleRoleChange = (name: string) => {
    setRoleName(name);
    // Reset permissions to the role's defaults when the role changes.
    const role = roles.find((r) => r.name === name);
    setSelectedPerms(role ? [...role.permissions] : []);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {editing
              ? "Update details, role and permissions for this admin user."
              : "Create a new admin user and assign their role and permissions."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="bg-[#962E3C] text-white hover:bg-[#7f2632]"
            >
