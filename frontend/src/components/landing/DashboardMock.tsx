...
export function HeroDashboardMock({ title, rows, linkText }: HeroDashboardMockProps) {
  return (
    <motion.div
      ...
      className="w-full max-w-md rounded-2xl border border-border bg-foreground/[0.03] p-5 shadow-2xl shadow-black/40 backdrop-blur"
    >
      ...
      <div className="mt-3 flex items-center gap-2 rounded-lg border border-border bg-background/50 px-3 py-2.5 text-sm text-muted-foreground">
...
export function ToolkitDashboardMock({ title, stats, caption }: { title: string; stats: StatColumn[]; caption: string }) {
  return (
    <motion.div
      ...
      className="mx-auto max-w-3xl overflow-hidden rounded-2xl border border-border bg-foreground/[0.03] shadow-2xl shadow-black/40"
    >
      <div className="flex items-center gap-1.5 border-b border-border px-5 py-4">
        <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
        <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
        <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
        <span className="ml-2 text-sm font-medium text-foreground/80">{title}</span>
      </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-border">
        {stats.map((s) => (
          <div key={s.label} className="px-5 py-6">
            ...
            <p className={`mt-3 h-4 w-10 rounded-full ${s.accent ? "bg-primary/70" : "bg-foreground/70"}`} />
          </div>
        ))}
      </div>
